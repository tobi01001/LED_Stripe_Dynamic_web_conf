#include "DotColWipeEffect.h"
#include "../WS2812FX_FastLed.h"
#include "../EffectHelper.h"

bool DotColWipeEffect::init(WS2812FX* strip) {
...
void DotColWipeEffect::initializeBars(WS2812FX* strip) {
    ...
    for (uint8_t i = 0; i < numBars; i++) {
        ...
        uint8_t divisor = random8(1, 3);
        uint8_t multiplier = random8(3, 6);
...
void DotColWipeEffect::updateBarSpeeds(WS2812FX* strip) {
    auto seg = strip->getSegment();
...
uint16_t DotColWipeEffect::calculateBarPosition(uint8_t barIndex, uint8_t waveType, WS2812FX* strip) {
    ...
    // Apply different wave functions based on wave type
...
void DotColWipeEffect::updateBarColor(uint8_t barIndex, uint16_t position, WS2812FX* strip) {
...
uint16_t DotColWipeEffect::update(WS2812FX* strip) {
    ...
    // No background fade for color wipe effect (FADE_AMOUNT = 0)
...
const __FlashStringHelper* DotColWipeEffect::getName() const {
    return F("Dots Color Wipe");
}

uint8_t DotColWipeEffect::getModeId() const {
    return FX_MODE_DOT_COL_WIPE;
}

void DotColWipeEffect::cleanup() {
    if (barStates) {
        delete[] barStates;
        barStates = nullptr;
    }
...
REGISTER_EFFECT(FX_MODE_DOT_COL_WIPE, DotColWipeEffect)